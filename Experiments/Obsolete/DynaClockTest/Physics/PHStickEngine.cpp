...
bool PHStick::AddChildObject(SGObject* o, SGScene* s){
	...
	if (!fr && DCAST(PHSolid, o)) fr = ((PHSolid*)o)->frame;
	if (fr){
		if(!frame[0]){
			frame[0] = fr;
		}else if (!frame[1]){
...
void PHStickEngine::After(SGScene* scene, float dt, unsigned count, CDFramePairWithRecord* fp){
...
void PHStickEngine::Loaded(SGScene* scene){
	for(unsigned int i=0; i<sticks.size(); ++i){
		for(int j=0; j<2; ++j){
			...
			if (solid) sticks[i]->frame[j] = solid->frame;
		}
	}
	contactEngine = NULL;
...
class PHStickLoader:public FIObjectLoader<PHStick>{
	virtual void LoadData(FILoadScene* ctx, UTRef<PHStick>& s){
		s = new PHStick;
		ctx->docs.Top()->GetData(s->force);
	}
};
DEF_REGISTER_LOADER(PHStick);

class PHStickEngineLoader:public FIObjectLoader<PHStickEngine>{
	virtual void LoadData(FILoadScene* ctx, UTRef<PHStickEngine>& se){
		se = new PHStickEngine;
	}
};
...
public:
	...
	virtual void Save(FISaveScene* ctx, SGObject* arg){
		...
		for(PHStickEngine::Sticks::iterator it = se->sticks.begin(); it != se->sticks.end(); ++it){
			...
			docStick->SetData((*it)->force);
			doc->AddChild(docStick);
			docStick->AddChild(ctx->CreateDocNode("Frame", (*it)->frame[0], true));
			docStick->AddChild(ctx->CreateDocNode("Frame", (*it)->frame[1], true));
		}
	}
};
