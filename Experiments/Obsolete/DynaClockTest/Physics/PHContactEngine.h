...
protected:
	...
	std::vector< std::pair<int, int> > inactiveList;
...
public:	
	...
	///	計算時間計測のためのタイマー
	WBPreciseTimer timerReflex, timerFriction;
	///
	PHContactEngine();
	///	子オブジェクトの追加
	...
	FramePairRecord* GetFramePairRecord(SGFrame* fr1, SGFrame* fr2){
		return UTRef<FramePairRecord>(collisionEngine->GetFramePair(fr1, fr2).records[framePairRecordPos]);
	} 
	///	アクティブでないフレーム対を追加
...
protected:
	...
	void Before(SGScene* scene, float dt, unsigned count, CDFramePairWithRecord* fr);
	...
	/**	フレーム同士が衝突している場合，衝突解析が終わってからフレーム対につき1度呼び出される．
		抗力の調整，摩擦力計算を行う	*/
	void After(SGScene* scene, float dt, unsigned count, CDFramePairWithRecord* fr);
	///	抗力計算
