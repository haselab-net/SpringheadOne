...
void PHWarpEngine::Loaded(SGScene* scene){
	...
	for(unsigned i=0; i<warps.size(); ++i){
		...
		if (warp->warpZoneFrame){
			...
			ce->AddFrame(sols[0]->frame);
			f1 = ce->GetFrameID(sols[0]->frame);
			f2Start = 1;
		}
		for(unsigned j=f2Start; j<sols.size(); ++j){
			ce->AddFrame(sols[j]->frame);
			f2 = ce->GetFrameID(sols[j]->frame);
...
void PHWarpEngine::After(SGScene* scene, float dt, unsigned count, CDFramePairWithRecord* fp){
	...
	fpr->solid[0]->SetPosition(fpr->position);
	...
	if(fpr->solid[1]){
		fpr->solid[1]->SetPosition(fpr->position);
...
class PHWarpLoader:public FIObjectLoader<PHWarp>{
	virtual void LoadData(FILoadScene* ctx, UTRef<PHWarp>& w){
		w = new PHWarp;
...
void PHWarpSaver::Save(FISaveScene* ctx, SGObject* arg){
	...
	doc->SetData(info);
	if (warp->warpZoneFrame){
		doc->AddChild(ctx->CreateDocNode("Frame", warp->warpZoneFrame, true));
	}
	for(unsigned i=0; i<warp->solids.size(); ++i){
		doc->AddChild(ctx->CreateDocNode("Solid", warp->solids[i], true));
	}
}
...
class PHWarpEngineLoader : public FIObjectLoader<PHWarpEngine>{
	virtual void LoadData(FILoadScene* ctx, UTRef<PHWarpEngine>& we){
		we = new PHWarpEngine;
	}
};
