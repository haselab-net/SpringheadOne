...
public:
	...
	//PHJoint* joints[JOINT_NUM];
	Spr::PHJoint *joWaist1, *joWaist2, *joWaist3, *joNeck1, *joNeck2, *joNeck3,
	...
	THuman();
	virtual ~THuman();
	bool Connect(Spr::UTRef<Spr::SGScene> scene);
	void ConnectSolid(Spr::PHSolid*& solid, const char* name, Spr::UTRef<Spr::SGScene> scene);
	void ConnectJoint(Spr::PHJoint*& joint, const char* name, Spr::UTRef<Spr::SGScene> scene);
	...
	// osada作
	void SaveTorque(float time, float dt);	//各関節のトルクを時系列で保存する(jojoに)
	std::vector<double> jojo[30];
	Spr::Vec3f GetMassCenterPosition();		//VHの重心を計算する
	bool Draw(Spr::GRRender* render);		//VHの重心を描画

	// テスト用
	bool checkPart;
	std::vector<double> TargetAngle[30];
	std::vector<double> NowAngle[30];


protected:
	void SaveOneAngle(TAngle& a, Spr::PHJoint* j);
	void LoadOneAngle(TAngle& a, int c, Spr::PHJoint* j);
	void SaveOneTorque(TAngle& a, int c, Spr::PHJoint* j);
};
