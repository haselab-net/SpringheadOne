...
Spr::GRMaterialData Mat(Vec4f c){
	return Spr::GRMaterialData(c, c, c,	c, 0.0f);
}
void TDiffFile::Draw(Spr::GRRender* render, THuman& human, float time){
	if (!IsLoaded()) return;
	...
	for(int lr=0; lr<2; ++lr){
		...
		for(int i=0; i<13; ++i){
			if (IsValid(pos[i])){
				render->DrawDirect(GRRender::POINTS, pos+i, pos+i+1);
			}
		...
		for(int i=0; i<12; ++i){
			if (IsValid(pos[i]) && IsValid(pos[i+1])){
				render->DrawDirect(GRRender::LINES, pos+i, pos+i+2);
			}
		}
		//	バネの端点の表示
		...
		render->SetLineWidth(5);
		for(int i=0; i<human.springs[lr].size() && human.springs[lr][i].solid; ++i){
			Vec3f pos = human.springs[lr][i].GetPos();
			render->DrawDirect(GRRender::POINTS, &pos, &pos+1);
		}
		render->SetLineWidth(1);
		for(int i=0; i+1<human.springs[lr].size() && human.springs[lr][i+1].solid; ++i){
			Vec3f p[2];
			p[0]= human.springs[lr][i].GetPos();
			p[1] = human.springs[lr][i+1].GetPos();
			render->DrawDirect(GRRender::LINES, p, p+2);
		}
	}
}
void TDiffFile::TRecords::Draw(GRRender* render, float time){
	...
	for(int i=0; i<pos.data.size(); ++i){
		if (pos.data[i].norm() < 9000){
			render->DrawDirect(GRRender::POINTS, pos.data.begin()+i, pos.data.begin()+i+1);
		}
		render->SetLineWidth(3);
	}
	render->SetLineWidth(1);
	for(int i=0; i<pos.data.size(); ++i){
		if (pos.data[i].norm() < 9000 && pos.data[i+1].norm() < 9000){
			render->DrawDirect(GRRender::LINES, pos.data.begin()+i, pos.data.begin()+i+2);
		}
	}
}
