// VHIndependenceView.cpp : CVHIndependenceView クラスの動作の定義を行います。
//
...
IMPLEMENT_DYNCREATE(CVHIndependenceView, CView)

BEGIN_MESSAGE_MAP(CVHIndependenceView, CView)
	//{{AFX_MSG_MAP(CVHIndependenceView)
	ON_WM_KEYDOWN()
	ON_WM_ERASEBKGND()
	ON_WM_TIMER()
	ON_WM_DESTROY()
...
/////////////////////////////////////////////////////////////////////////////
// CVHIndependenceView クラスの構築/消滅

CVHIndependenceView::CVHIndependenceView()
{
	// SPIDARの初期化を行う
	ReadRegistry();
	WriteRegistry();
...
void CVHIndependenceView::ReadRegistry(){
...
void CVHIndependenceView::WriteRegistry(){
...
BOOL CVHIndependenceView::PreCreateWindow(CREATESTRUCT& cs)
{
...
/////////////////////////////////////////////////////////////////////////////
// CVHIndependenceView クラスの描画

void CVHIndependenceView::OnDraw(CDC* pDC){
	CVHIndependenceDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	app.Draw();
	//vhDialog.Draw(app.render, app.crBalance);
}
/////////////////////////////////////////////////////////////////////////////
// CVHIndependenceView クラスの診断

#ifdef _DEBUG
void CVHIndependenceView::AssertValid() const
{
	CView::AssertValid();
}

void CVHIndependenceView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}

CVHIndependenceDoc* CVHIndependenceView::GetDocument() // 非デバッグ バージョンはインラインです。
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CVHIndependenceDoc)));
	return (CVHIndependenceDoc*)m_pDocument;
}
#endif //_DEBUG

void CVHIndependenceView::InitHis(std::string hisList, float ps, float fs){
	app.ClearHis();
...
/////////////////////////////////////////////////////////////////////////////
// CVHIndependenceView クラスのメッセージ ハンドラ
void CVHIndependenceView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags){
...
void CVHIndependenceView::ThrowSphere(float r){
	...
	solid->SetVelocity(-afSphere.Ez() * 14.0f);
...
BOOL CVHIndependenceView::OnEraseBkgnd(CDC* pDC) 
{
	// MFCはデフォルトで背景を白く塗る．ちらつくので何もしない関数でオーバーライド．
	return true;
}

void CVHIndependenceView::OnInitialUpdate() 
{
...
void CVHIndependenceView::OnTimer(UINT nIDEvent){
	if (nIDEvent == FWApp::TIMER_DRAW){
		Invalidate();
	}
}

void CVHIndependenceView::OnDestroy() 
{
	CView::OnDestroy();
...
void CVHIndependenceView::OnD3d(){
	app.CreateD3DRender();
}

void CVHIndependenceView::OnGL(){
	app.CreateGLRender();
}

BOOL CVHIndependenceView::PreTranslateMessage(MSG* pMsg) 
{
	// TODO: この位置に固有の処理を追加するか、または基本クラスを呼び出してください
...
void CVHIndependenceView::OnHisConfig(){
...
void CVHIndependenceView::OnVhDialog() 
{
	// TODO: この位置にコマンド ハンドラ用のコードを追加してください
