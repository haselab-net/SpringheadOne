...
{
//10/27 JEONG          5/11 ytakeyama scribbled
	...
	glPushMatrix();
		...
		if (h > 0)
		{
			/*// ORIENT PARENT'S Y AXIS TO POINT AT CHILD   とりあえず保存
			float phi, theta;
			...
			// Vec(x,y,z) を Vec'(0, abs(Vec), 0) に見えるように回すのだ
			float phi, theta;
			phi = acos(x/h)*180.0/M_PI;
			//theta = acos(y/h)*180.0/M_PI;   //？？
			theta = atan(h/y)*180.0/M_PI;     //こうなのでは？
			if(z<0) phi = -1 * phi;           //これ必要なのでは？　ありえないから不要？
			//↓右ねじだから逆なのでは？
			glRotatef(-1*phi, 0.0f, 1.0f, 0.0f);		// y  
	       	glRotatef(-1*theta, 0.0f, 0.0f, 1.0f);		// z

			//if (z == 0) d = -d;		// hack adjustment  不要？
		}
		else
		{
			glDisable(GL_LIGHTING);
			if (y < 0){
				// d = -d;//だめ。affineで使いたいから方向重要。
				//↓ではダメな気がする
...
//-----------------------------------------------------------------------
void myglSolidFrustum(short Nxy,float ratio)//円錐台
{   //下底の中心を物体の中心
	...
	{   theta = theta0*(double)i;
		p[i][0] = (float)(r*cos(theta)*ratio); //上底のx成分
		p[i][1] = (float)(r*sin(theta)*ratio);       //ｙ成分
		p[i][2] = 1.0f;                              //ｚ成分
		p[i+Nxy][0] = (float)(r*cos(theta)); //下底のx成分
		p[i+Nxy][1] = (float)(r*sin(theta)); //y成分
		p[i+Nxy][2] = 0.0f;                  //ｚ成分
	}
