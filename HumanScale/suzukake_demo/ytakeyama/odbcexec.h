...
// z position is compared
	   ...
	   // init_pose fixed or randomly able to select(dance, waiting)
		   ...
		   {
       			SQLGetData(hstmt1, 1, SQL_C_LONG,&ret[0], sizeof(long), &cbValue[0]);
				SQLGetData(hstmt1, 2, SQL_C_LONG,&ret[1], sizeof(long), &cbValue[1]);
				row_cnt = row_cnt + 1;
		   }
		}

	   ret[3] =row_cnt;
      // if (row_cnt == 0 ) ret[0] = init_type;
...
int load_basic(int m_type, int mm_type)
{
   ...
   while ( (retcode = SQLFetch(hstmt1) ) != SQL_NO_DATA )
   {
        ...
        SQLGetData(hstmt1, 2, SQL_C_LONG,&J_cnt, sizeof(long), &cbValue[1]);
   ...
   if (m_type !=0)  f_frame = pre_frame + f_frame;
   
   printf("total_frame = %d, pre_num= %d, motion_name= %d %s\n", f_frame, pre_frame, mm_type, szName);
...
int load_model(int model_type)
{
   ...
   retcode = SQLBindCol(hstmt1, 1, SQL_C_LONG, &tt_bone1[0], sizeof(long), &cbValue[0]);
   
   while ( (retcode = SQLFetch(hstmt1) ) != SQL_NO_DATA )
   {
         retcode = SQLGetData(hstmt1, 2, SQL_C_LONG, &tt_bone1[1], sizeof(long), &cbValue[1]);
		 
         SQLGetData(hstmt1, 3, SQL_C_CHAR, szName, MAXBUFLEN, &cbValue[2]);
         //strcpy(tt_bone.name, (char *)szName);
		
		 SQLGetData(hstmt1, 4, SQL_C_DOUBLE,&tt_bone2[0], sizeof(double), &cbValue[3]);
		 SQLGetData(hstmt1, 5, SQL_C_DOUBLE,&tt_bone2[1], sizeof(double), &cbValue[4]);
		 SQLGetData(hstmt1, 6, SQL_C_DOUBLE, &tt_bone2[2], sizeof(double), &cbValue[5]);
		 //tt_bone.b_trans*=(1/39.37); 
	
		 SQLGetData(hstmt1, 7, SQL_C_LONG, &tt_bone1[2], sizeof(long), &cbValue[6]);

 		 Create_Body(tt_bone1, tt_bone2);
	}
