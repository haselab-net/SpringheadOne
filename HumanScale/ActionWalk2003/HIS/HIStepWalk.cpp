...
void HIStepWalk::Step(float turnVel){
	...
	//	1回前に直してみる→(5回前にサンプリングした値との比較をして、)重心の変化の大きさをdCenterOfGravityとする
	dCenterOfGravity = (centerOfGravity[0] - centerOfGravity[CENTER_OF_GRAVITY_LEN-1]).norm() / (dt*(CENTER_OF_GRAVITY_LEN-1));
//	一回前は無理かも。回転しまくる可能性大
//	dCenterOfGravity = (centerOfGravity[0] - centerOfGravity[1]).norm() / (dt);
...
*/  else
	...
	if(statusOfUser != 7 && sumSensors[0] < sumSensorAve * 0.3 && sumSensors[1] < sumSensorAve * 0.3 && sumSensorMax[1].value > 1.3){
		statusOfUser = 7;
	}
...
//--------------サイドステップ検出-------------------//
	...
	if(centerOfGravity[0].norm() > sideStepPoint && dCenterOfGravity < 0.1){
		if(!sideStepFlag){
			sideStepFlag = true;
