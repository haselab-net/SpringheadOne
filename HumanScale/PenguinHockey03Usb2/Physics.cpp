...
//#else
//			HaColObject::afObj.Pos() = spidar.GetPos()*2;
			HaColObject::afObj.Pos() = spidar.GetPos()*1.5f;
			HaColObject::afObj.PosZ() -= 0.6f;		
			HaColObject::afObj.PosY() -= 0.15f;
		}
//#endif
		if (env.bHaptics) {
			...
			env.dRatio += ratio;
			force.clear();
			spidar.SetForce(force,ratio);			//力提示はゼロ
		}
...
void Register_Mouse() {
	cout << "マウスをSPIDARとして使用します(未実装)" <<endl;
}
...
void ChrSetPuck(int num) {
	Affinef afPuck;
	afPuck.Pos() = InitPuckPos[num];
	afPuck.Rot() = Matrix3f(ex,ey,ez);
	objects.Push(new HaPuck(afPuck));
}
