...
protected:
	...
	Vec2f nowDCenterG;              ///<    関数FootSide()用.
	Vec2f lastDCenterG;             ///<    関数FootSide()用.履歴用
	float nowDCenterAngle;          ///<    関数FootSide()用.
	float lastDCenterAngle;         ///<    関数FootSide()用.
	...
	FlagSumSensor sumSensorMax;
	FlagSumSensor sumSensorBig;
	FlagSumSensor sumSensorMin;
	FlagSumSensor sumSensorSmall;

	int statusOfUser;               ///<	ユーザの状態　0：未使用　1：直立　2：前進　3：後退　4：走る　5：右サイドステップ　6：左サイドステップ　7：ジャンプ　8：屈伸　9：屈伸から戻る
	...
	int sumMaxMinSetTime;           ///<	sumMaxMinSetTime/1000秒の間だけsumSensorMax,sumSensorMinの値を保持. 値は今のところ適当（500）
