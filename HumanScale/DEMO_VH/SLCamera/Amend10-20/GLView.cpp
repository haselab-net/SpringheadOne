...
void TGLView::CheckEyeAngle(){
	MainAngleSide = MainAngleSide*0.8;
	MainAngleVert = MainAngleVert*0.7;
	if(MainAngleVert > 0)
		MainAngleVert = MainAngleVert-abs(MainAngleSide)*0.002;
	else
		MainAngleVert = MainAngleVert+abs(MainAngleSide)*0.002;

	if(abs(PreMainAngleSide - MainAngleSide) < 3)
		MainAngleSide = PreMainAngleSide;
...
void TGLView::SelectAngle(){
	...
	if( BallPosChangeTotal.norm() > 0.1){
...
void TGLView::SelectRotateTurn(){
	//目だけを回転させるか、目の後に顔を回転させるか。
	if(abs(MainAngleVert) > 40 || abs(MainAngleSide) > 40){
		RotateTurn = 1;
	}
...
void TGLView::BallIsHolding(){
	...
	if(!hold){
		if(sp_info.speed > BallSpeedThreshold && ballSpeed.Z() > 0)
		{
			BBall.chk_shoot = SHOOTED; 
			BBall.chk_state = THROWING;
		}
...
/*
	...
	glPushMatrix();										//shadow
		glDisable(GL_CULL_FACE);						//shadow
			glMultMatrixf((GLfloat *)ShadowMat2);		//shadow
	        drawBody(mydb.vh.Anim.cur_frm);				//shadow
		glEnable(GL_CULL_FACE);							//shadow
	glPopMatrix();										//shadow
  ...
  //ボールのステンシル付け
	...
	glEnable(GL_BLEND);											//shadow
		...
		glDisable(GL_DEPTH_TEST);								//shadow
			...
			DrawFloor();										//shadow
			glColor4f(0.1f, 0.1f, 0.1f, 1.0f);					//shadow
			DrawFloor2();										//shadow
		glEnable(GL_DEPTH_TEST);								//shadow
...
///////////////////////////////////////////
 ...
 MyVector vLight2(-1, 50, 0);							//shadow
//vLightが二つ？jeong
...
//generate the shadow matrix
	...
	CalcShadowMat(ShadowMat2, nGround2, vLight2);			//shadow
