...
//ytakeyama 視点位置の送信のため。SLCMoveを送信する
class CmdMove:public Command{
public:
	TGLView* view;
	CmdMove(TGLView* v):view(v),Command(&(v->cmdSocket)){}
	int Id()const{ return 5; }  // ID番号に注意！
	int Receive(char* data){
		view->Render->SLCMove = *((Vec3f*)data);
		//cout << "[Sync num (sum)] " << num << endl;
		//view->SyncBuffer[num] = 1;
		//view->ReadyNum = num;
		return sizeof(view->Render->SLCMove);
	}
	void Send(Vec3f SLCMovee){
		SendId();
		Vec3f eyedir = SLCMovee;
		//cout << "Yeah" << endl;
		SendData(&eyedir, sizeof(eyedir));
		cmdSocket->Flush();
	}
...
//		birdsView = true;
	...
	ini_radius = atof( temp_str );//ytakeyama
	...
	ini_eyeheight = atof( temp_str );//ytakeyama
	afEye = Affinef(0,ini_eyeheight,0);
	GetPrivateProfileString("WORLD","UnitPer1M","0",temp_str,100,ini_file); // 1m = ?? unit
	UnitPerM = atof( temp_str ); 
...
////////////////////////
	...
	*/
	...
	cmdSocket.Register(new CmdMove(this));//ytakeyama
...
void TGLView::OnDraw(){
	...
	glLoadIdentity();
	gluPerspective(fov_v*1.5, (float)(currentWidth)/(float)(currentHeight) , 0.1f , 1000000.0f);
	glMatrixMode(GL_MODELVIEW);
	// end

	//glClearColor(clearColor.X(), clearColor.Y(), clearColor.Z(), 1);
	glClearColor(0.5, 0.0, 0.0, 1);//ytakeyama
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	
	glLoadIdentity();

	static Affinef R180(Rad(180),'y');
	afC = afBody; //ytakeyama
	
	//ytakeyama ラベリングカメラの値から描画位置をずらす計算	
	afC.PosX() += 10 * (UnitPerM / 10.0) * (Render->SLCMove[2] * afBody.EzX() + Render->SLCMove[0] * afBody.EzZ());
	afC.PosY() += 10 * (UnitPerM / 10.0) * Render->SLCMove[1];
	afC.PosZ() += 10 * (UnitPerM / 10.0) * (Render->SLCMove[2] * afBody.EzZ() - Render->SLCMove[0] * afBody.EzX());
		//afC.PosX() += 10 * -1;
		//cout << "keymoveZERO = " << Render->keyMove[0] << endl;
		//cout << "SLCMove = " << Render->SLCMove << endl;
	afC = stVision.ShiftCamera(afC, local_eyedir, UnitPerM); // ステレオ誤差量をずらしている
	Render->IdirX = local_eyedir.X();
	Render->IdirY = local_eyedir.Y();
	//end
...
//	glutPostRedisplay();
	...
	*/
	...
	//Flip();  //   <- ytakeyama comment out
	
	//ytakeyama
	buf->Unbind();
	glDrawBuffer(GL_BACK);
	Render->TwoPassRend(cpNum, fov_v, R, currentWidth, currentHeight);
	//end
...
{
	static Affinef R180(Rad(180),'y');//ytakeyama
	switch(key){
	case '0':
	...
	case 'D':
		...
		break;
	case 'e':
		Render->keyMove[0] += 0.1;
		cout << "視点位置" << Render->keyMove[0] * 100 <<"cm右へ移動" << endl; 
		break;
	case 'E':
	...
	case 'i':
		...
		Render->keyMove = Vec3f(0.0,0.0,0.0);
		break;
	case 'j':
		afBody.Pos().Y() += 10;
		break;
	case 'l':
		...
		break;
	case 'k':
		afBody = afBody * R180;//ytakeyama test check2.wrl用
		break;
	case 'K':
		afBody.PosX() = afBody.PosX() + 20.0;//ytakeyama test
		break;
...
//			turnDataFile = new std::ofstream(_itoa(printTurnDataTime,printTurnData,10));
		...
		break;
	case 'q':
		Render->keyMove[2] -= 0.1;
		cout << "視点位置" << Render->keyMove[2] * -100 <<"cm前へ移動" << endl; 
		break;
	case 'r':
	...
	case 'x':
		float inputheight;
		cout << "目の高さと1.6ｍの差を入力してください（単位はｍ）：" << ends;
		cin >> inputheight;
		//cout << inputheight << endl;
		Render->keyMove[1] = inputheight;
		break;
	case 'y':
		Render->keyMove = Vec3f(0.0,0.0,0.0);
		break;
...
void TGLView::OnOpen(void){
	...
	Render = new TPRender();    // newじゃないとだめかも。
	Render -> initialize((double *) setting, cpNum);
	//Render->SLCMove = Vec3f(0.0, - 100.0 / UnitPerM * (ini_radius + ini_eyeheight), 0.0);//効率悪くてすいません
	Render -> fovCopy = fov_v;
	//end
...
//	if (!isWalking){
	if(hiStep.GetVelocity() < hiTurn.GetStopSpeed()) {
		...
		VecBodyYT = stVision.StereoLC(labelP,afBody,&SLCvf);
		//cout << "x =" <<stVision.vecSLC_copy.X() << endl;
		//Render->SLCMove = Vec3f(1.0, 0.0, 0.0);
		VecBodyYT[1] = 300.0 + VecBodyYT[1] - 100.0 / UnitPerM * (ini_radius + ini_eyeheight);
		if(bSender && SLCvf == 0){
			Render->SLCMove = VecBodyYT / 100 + Render->keyMove;
			//cout << "rewrite SLCMove" << endl;
		}
		//cout << "slcmove = " << Render->SLCMove << endl;
		//end
	}
...
*/			
	...
	if (bSender && !b){
		...
		CmdMove cmdMove(this);//ytakeyama
		cmdMove.Send(Render->SLCMove);//ytakeyama
		//cout << "slcmove = " << Render->SLCMove << endl;
	}
