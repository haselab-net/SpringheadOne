...
bool TGLView::LoadSettings(){
	...
	ostrfn << ".\\Data\\CameraParameter\\sf" << projNum << '\0';
...
//		birdsView = true;
	...
	// iniファイルからデータを読み込む (場所を移動：\config)
	char temp_str[100];
	char ini_file[100];
	GetCurrentDirectory( 100,ini_file );
	strcat( ini_file,"\\config\\setting.ini" );    // この場所は固定
	cerr << "[INI File] : " << ini_file << endl;

	// 起動ディレクトリを取得  (PCクラスタと制御PCでは実行パスが異なるため)
	GetCurrentDirectory( 100, ini_dir );


	// ディレクトリの読み取り
    GetPrivateProfileString("DIRECTORY","PREFIX",NULL,Prefix,100,ini_file); // 実行ディレクトリ
    PathEndFinisher(Prefix, sizeof(Prefix));  // 文字列の終端を"\"に統一
	cerr << "[PREFIX]" << Prefix << endl;
    GetPrivateProfileString("DIRECTORY","ProjectName",NULL,ProjectName,100,ini_file); // プログラムの場所
    PathEndFinisher(ProjectName, sizeof(ProjectName));
	cerr << "[ProjectName] " << ProjectName << endl;
    GetPrivateProfileString("DIRECTORY","ProgramPath",NULL,ProgramPath,100,ini_file); // プログラムの場所
    PathEndFinisher(ProgramPath, sizeof(ProgramPath));
	cerr << "[ProgramPath] " << ProgramPath << endl;
	GetPrivateProfileString("DIRECTORY","ProgramName",NULL,ProgramName,100,ini_file); // プログラム名
    PathEndFinisher(ProgramName, sizeof(ProgramName));
	cerr << "[ProgramName] " << ProgramName << endl;
    GetPrivateProfileString("DIRECTORY","VrmlPath",NULL,VrmlPath,100,ini_file); // プログラムの場所
    PathEndFinisher(VrmlPath, sizeof(VrmlPath));
	cerr << "[VrmlPath] " << VrmlPath << endl;
	GetPrivateProfileString("DIRECTORY","SoundPath",NULL,SoundPath,100,ini_file); // プログラム名
    PathEndFinisher(SoundPath, sizeof(SoundPath));
	cerr << "[SoundPath] " << SoundPath << endl;
...
void TGLView::OnSize(int width, int height){
	...
	//gluPerspective(fov_v, (float)(height)/(float)(width) , 1.0f , 100000000.0f);
/*	
	Affinef af;
...
void TGLView::OnOpen(void){
	...
	if( easyds->IsAvailable() ){  // Pathの指定方法は考えなければ…(カレントpathを一時的に動かす？)
		easyds->Waves(0)->LoadFromFile( ".\\Data\\Sound\\collision.wav" );
		easyds->Waves(0)->SetSynchronous(3);
		easyds->Waves(1)->LoadFromFile( ".\\Data\\Sound\\fall.wav" );
...
*/			
	...
	static char* stop_file = NULL;

	if(stop_file == NULL){  // stopファイルの場所を指定
	  stop_file = tmp_char;
	  strcpy(stop_file, ini_dir);  // クラスタでは起動ファイルとPrefixが異なるため
      strcat(stop_file, "\\stop");
	  cerr << "[stop] : " << stop_file << endl;
	}

	stopFileDetectCounter ++;
	if (stopFileDetectCounter > 10){
		if (_access(stop_file, 00) != -1){
			Exit();
		}
...
// 文字列の終端を"\"で閉じる　（ただし文字列の実体にメモリ的余裕があること）
char* TGLView::PathEndFinisher(char* string, int length){
	//int length;
	int i = 0;

	while(string[i] != 0) i++;
	if(i > length - 2) return NULL;  // 配列に余裕がなければ終了
	else 
		if(string[i-1] != '\\'){
			string[i] = '\\';
			string[i+1] = 0;
