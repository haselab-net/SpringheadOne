...
// Texture mapM_PIng array(^ ^), this files must be same size(ex;256*256)
	...
	if((texRec[0]=LoadBMPFile("../vh_data/carp2.bmp")) && 
	   (texRec[1]=LoadBMPFile("../vh_data/soo.bmp"))) //texture
	{   
...
//10/27 JEONG
	...
	glPushMatrix();
		...
		if (h > 0)
		{
			...
			// Vec(x,y,z) Ç Vec'(0, abs(Vec), 0) Ç…å©Ç¶ÇÈÇÊÇ§Ç…âÒÇ∑ÇÃÇæ
			float phi, theta;
			phi = acos(x/h)*180.0/M_PI;
			theta = atan(h/y)*180.0/M_PI;     
			if(z<0) phi = -1 * phi;           
			//Å´âEÇÀÇ∂ÇæÇ©ÇÁãtÇ»ÇÃÇ≈ÇÕÅH
