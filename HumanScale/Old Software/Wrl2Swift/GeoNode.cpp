...
static map<string,UTRef<TGeoNode> > dnMap;
...
//----------------------------------------------------------------------------
//	TGeoNode
TGeoNode::TGeoNodes TGeoNode::allNodes;
void TGeoNode::Draw(){
...
TGeoNodeBuilder::TGeoNodeBuilder(){
	...
	nodeStack.push_back(new TGeoNode);
...
void TGeoNodeBuilder::FoundNamed(TNode *pn){
	...
	node->vrmlNode = pn;
	TGeoNode::allNodes.push_back(node);
...
void TGeoNodeBuilder::Build(TSts* sts){
	...
	for(TGeoNode::TGeoNodes::iterator it = TGeoNode::allNodes.begin(); it != TGeoNode::allNodes.end(); ++it){
		curNode=*it;
		TNode* pn = curNode->vrmlNode;
		pn->GetType()->EnumTypeNode(pn);
//		for(vector<Vec3f>::iterator it2 = curNode->vertices.begin(); it2 != curNode->vertices.end(); ++it2){
//			cout << *it2 << endl;
//		}
	}
	TType::listener = NULL;
...
bool TGeoNodeBuilder::FoundTypeNode(TNode *pn){
	if (strcmp(pn->GetType()->GetName(), "IndexedFaceSet")==0){
		vector<int> v;
		vector<float> p;
		TSts* pSts = 0;
