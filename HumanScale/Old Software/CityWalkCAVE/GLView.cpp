...
long TimeYT = 0; //時間カウンタ
int IdouMode = 0; // 移動制御用
int PCnameYT = 0; //画面選択用
...
//		birdsView = true;
	...
	GetPrivateProfileString("AVATOR","PCnameYT","0",temp_str,100,ini_file); //画面選択
	PCnameYT = atof( temp_str );
...
//		afCam = afProjector;// * Affinef( Rad(-viewTurn),'z' );
		...
		if (PCnameYT == 1){//PCの役割によって描画角度を変える
			//glRotated(-90.0, 0.0, 1.0, 0.0);
			afCam = afBody * Affinef(0,13.5,0) * Affinef(Rad(90), 'y');
		}else if (PCnameYT == 2){
			//glRotated(90.0, 0.0, 1.0, 0.0);
			afCam = afBody * Affinef(0,13.5,0) * Affinef(Rad(-90), 'y');
		}else if (PCnameYT == 3){
			//glRotated(90.0, 0.0, 1.0, 0.0);
			afCam = afBody * Affinef(0,13.5,0) * Affinef(Rad(-90), 'x'); /* * Affinef( Rad(-viewTurn),'z' );*/
		}
...
void TGLView::OnSize(int width, int height){
	...
	// CAVE用に変更（武山）
	if (PCnameYT == 0){
		glFrustum(-0.1375, 0.1375, -0.1375, 0.0458333333333333, 0.2, 3000);
	}else if(PCnameYT ==1){
		glFrustum(-0.109090909090909, 0.2909090909090909, -0.2, 0.066666666666, 0.2, 3000);
	}else if(PCnameYT == 2){
		glFrustum(-0.2909090909090909, 0.109090909090909, -0.2, 0.066666666666, 0.2, 3000);
	}else if(PCnameYT == 3){
		glFrustum(-0.2, 0.2, -0.109090909090909, 0.2909090909090909, 0.2, 3000);
	}

	/*
	if (PCnameYT == 0){
		glFrustum(-0.12, 0.12, -0.1375, 0.1125, 0.2, 3000);
	}else if(PCnameYT ==1){
		glFrustum(-0.065, 0.26, -0.15, 0.122727272727, 0.2, 3000);
	}else if(PCnameYT == 2){
		glFrustum(-0.26, 0.065, -0.15, 0.122727272727, 0.2, 3000);
	}*/
...
void TGLView::OnKeyboard(int key, int x, int y)
{
	...
	case 'D':
		...
		break;
	case 'e':
		afBody = afBody * Affinef(Rad(20.0f), 'Y');
		break;
	case 'E':
		...
		switch(IdouMode){
			case -1:
				...
				if (!(specialKey[GLUT_KEY_LEFT])&& !(specialKey[GLUT_KEY_RIGHT])){
					if (PCnameYT == 5){
						afBody.Pos().X() += afBody.EzZ() * 0.6;
						afBody.Pos().Z() -= afBody.EzX() * 0.6;
					}else if (PCnameYT == 6){
						afBody.Pos().X() -= afBody.EzZ() * 0.6;
						afBody.Pos().Z() += afBody.EzX() * 0.6;
					}else{
						afBody.Pos().Z() -= afBody.EzZ() * 0.6;
						afBody.Pos().X() -= afBody.EzX() * 0.6;
					}
				}
				break;
			case 1:
				if (PCnameYT == 1){
					afBody.Pos().X() += -0.333333333333;
				}else if (PCnameYT == 2){
					afBody.Pos().X() += 0.333333333333;
				}else{
			...
			case 3:
				if (PCnameYT == 1){
					afBody.Pos().X() += cos(Rad(60)) * -0.3333333333333;
					afBody.Pos().Z() += sin(Rad(60)) * 0.3333333333333;
				}else if (PCnameYT == 2){
					afBody.Pos().X() += cos(Rad(60)) * 0.3333333333333;
					afBody.Pos().Z() += sin(Rad(60)) * -0.3333333333333;
				}else{
			...
			case 5:
				if (PCnameYT == 1){
					afBody.Pos().X() += cos(Rad(15)) * -0.3333333333333;
					afBody.Pos().Z() += sin(Rad(15)) * 0.3333333333333;
				}else if (PCnameYT == 2){
					afBody.Pos().X() += cos(Rad(15)) * 0.3333333333333;
					afBody.Pos().Z() += sin(Rad(15)) * -0.3333333333333;
				}else{
			...
			case 7:
				if (PCnameYT == 1){
					afBody.Pos().X() += cos(Rad(-75)) * -0.3333333333333;
					afBody.Pos().Z() += sin(Rad(-75)) * 0.3333333333333;
				}else if (PCnameYT == 2){
					afBody.Pos().X() += cos(Rad(-75)) * 0.3333333333333;
					afBody.Pos().Z() += sin(Rad(-75)) * -0.3333333333333;
				}else{
