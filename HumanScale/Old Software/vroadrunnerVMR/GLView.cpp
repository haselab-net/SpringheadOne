...
#ifdef _GAME_11_30
#define PLAY_TIME 50 //ゲームオーバーまでのタイム
#else
#define PLAY_TIME 120 //ゲームオーバーまでのタイム
#endif //_GAME_11_30
...
//	cout << "afBody:" << afBody.Pos() << endl;
	...
	if(statusOfUser != lastStatusOfUser && statusOfUser == 7){
		float jumpPower = 1000.0f;//そのうちsumSensorの積分の関係式から出す。
		vel.Y() += jumpPower;
//		afBody.Pos().Y() = jumpPower;
	...
	// レーザーポインタ-->>
#ifdef USE_LASER_POINTER
	// (参考)USE_LASER_POINTが使われないときはマウス座標が使われる
	laserX = udpRecv->GetX();
	laserY = udpRecv->GetY();
	laserCaptured = udpRecv->IsCaptured();
#endif
	float innerScreenHeight = screenNear * tan( PI * ( fov_v / 2.0 ) / 180.0 ) * 2.0;
	float innerScreenWidth = windowWidth * innerScreenHeight / windowHeight;
	afLaser = Affinef(
		( laserX - windowWidth / 2.0 ) * innerScreenWidth / windowWidth,
		-( laserY - windowHeight / 2.0 ) * innerScreenHeight / windowHeight,
		-screenNear
	);
	Affinef afPoint = afCam * afLaser;
	vecLaser = afPoint.Pos() - afCam.Pos();
#ifdef _DEBUG_TKOBA
	cout << "LaserX:" << laserX << " , LaserY:" << laserY << endl;
#endif // _DEBUG_TKOBA
	if( pLaserNode ){
		pLaserNode->Step();
	}
	if( laserCaptured == false && lastLaserCaptured == true ){
		LaserAttack();
	}
	lastLaserCaptured = laserCaptured;
	// <<-- レーザーポインタ
...
//#ifdef _OKADA_TEST_APP
	//  動く床に乗ってるときはy方向の速度を０にする（衝突判定の代わり）
	if(bGravity) {
...
//#else //!_OKADA_TEST_APP
//	vel.Y() -= gravity * dt * gravity;
//#endif //_OKADA_TEST_APP

	vel = 0.88f * vel; //0.98 -> 0.88 //変更 katsu
	// 0.98だと、進みすぎる

#ifdef _DEBUG_KATSU
	cout << "vel_test:" << vel << endl;
#endif //_DEBUG_KATSU

	//---------------------------------------------------------------
	//	衝突判定を行い，反射の方向を求める．また，侵入をキャンセルする．

	// cpds	:TClosestPointDetectors
	// rnc	:TReflectNormalCalculator
	// penCancel	:TPenaltyCanceler
	...
	//Affinef afPoint;
	afPoint = afBody * afLHand;
	LHand_cpds.Detect((float*)afPoint.Pos(), hand_radius);		//	左手の最近傍点を求める
	afPoint = afBody * afRHand;
	RHand_cpds.Detect((float*)afPoint.Pos(), hand_radius);		//	右手の最近傍点を求める

	// 左手
	Vec3f lVecSum,rVecSum,bodyVecSum;
	for(TClosestPointDetectors::iterator it = LHand_cpds.begin(); it != LHand_cpds.end(); ++it){
	...
	// 右手
	for(TClosestPointDetectors::iterator it = RHand_cpds.begin(); it != RHand_cpds.end(); ++it){
		for(TClosestPoints::iterator itp = it->points.begin(); itp != it->points.end(); ++itp){
			Vec3d cp = (Vec3d&)itp->point;
	...
	penCancel.Clear();
	//	衝突面の列挙
	if (floorHeight + radius - afBody.Pos().Y() > 0){ //床にめり込んでいたら
		rnc.Add(Vec3d(0,1,0));
	...
	//	速度を反射させる．
	// 移動物体は、アバターが移動していなくてもぶつかってくるので、移動物体の移動速度なども考慮しないといけない。
	...
	vel -= (vel * normal) * normal; //速度を反射させずに停止


//cylinder制御
	Vec2d totalVector;       //傾き方向判定用ベクトル
	totalVector = Vec2d(0.0, 0.0);
	for(int i = 0; i < rnc.normals.size(); ++i){
		totalVector = totalVector + Vec2d(rnc.normals[i].X(), rnc.normals[i].Z());

#ifdef _DEBUG_IWASHITA
		std::cout << "X ==          " << rnc.normals[i].X() << std::endl;
		std::cout << "Z ==          " << rnc.normals[i].Z() << std::endl;
		std::cout << "\n\n\n" << std::endl;
#endif // _DEBUG_IWASHITA
	}

	// angle：プレイヤーの向きと斜面方向の角度の差
	float angle = atan2(totalVector.X(), totalVector.Y());
	if (specialKey[GLUT_KEY_LEFT]){
		playerAngle += Rad(5);
	}
	if (specialKey[GLUT_KEY_RIGHT]){
	...
	active = false;
	bool isJump;
	isJump = aJumpZone.GetisJump() || bJumpZone.GetisJump() || cJumpZone.GetisJump() ||
			 dJumpZone.GetisJump() || eJumpZone.GetisJump() || fJumpZone.GetisJump();
	if( isJump && !bLastIsJump ){
		PlaySound( SOUND_JUMP );
	}
	bLastIsJump = isJump;

//	if(rnc.normals.size() == 0) { isCollision = false; }
//	if(vel.Y() < 0){ isCollision = false; }
	if(totalVector.norm() > 1){ active = true; }
	pio.Control3(angle, active, isJump);
	//cylinder制御終了
...
void TGLView::GameStart()
{
	...
	if(i==0){
		...
		afBody.Pos().Y()=100;
		afBody.Pos().Z()=-270;
		afBody = afBody * Affinef(Rad(-90), 'Y');
#else
		afBody.Pos().Z()=-150;
#endif// _GAME_11_30
	}
