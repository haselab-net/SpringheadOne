...
int Cdirexec::load_motion(int frame_cnt, int mm_type)
{
    ...
    strcat(strSQL, buffer);
	strcat(strSQL, " and a.frame_no <= 500 and b.end_site =0 ");
   ...
   else
   {
	   ...
	   while ( (rc = SQLFetch(hstmt) ) != SQL_NO_DATA )
	   {
		   rc = SQLGetData(hstmt, 2, SQL_C_LONG, &bone_id, sizeof(long), &cbValue[1]);
		   ...
		   SQLGetData(hstmt, 3, SQL_C_DOUBLE,&temp_trans.x, sizeof(double), &cbValue[2]);
		   SQLGetData(hstmt, 4, SQL_C_DOUBLE,&temp_trans.y, sizeof(double), &cbValue[3]);
		   SQLGetData(hstmt, 5, SQL_C_DOUBLE,&temp_trans.z, sizeof(double), &cbValue[4]);

		   SQLGetData(hstmt, 6, SQL_C_DOUBLE,&temp_rot.z, sizeof(double), &cbValue[5]);
		   SQLGetData(hstmt, 7, SQL_C_DOUBLE,&temp_rot.x, sizeof(double), &cbValue[6]);
		   SQLGetData(hstmt, 8, SQL_C_DOUBLE,&temp_rot.y, sizeof(double), &cbValue[7]);

		   SQLGetData(hstmt, 9, SQL_C_LONG,&g_id, sizeof(long), &cbValue[8]);
		   SQLGetData(hstmt, 10, SQL_C_LONG,&idx, sizeof(long), &cbValue[9]);
		   
		   vh.Create_Motion(temp_trans, temp_rot, g_id, idx, frame_idx);
	   }
   }
   /* Clean up. */
   SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
