...
AUX_RGBImageRec* CMyTex::LoadBMPFile(char *filename)
{
	FILE *hFile = NULL;
...
// Texture mapM_PIng array(^ ^), this files must be same size(ex;256*256)
	...
	if((texRec[0]=LoadBMPFile("floor.bmp")) && 
	   (texRec[1]=LoadBMPFile("face.bmp")) && //texture
	   (texRec[2]=LoadBMPFile("target.bmp"))) //texture
	{   
...
// model foot 
void CMyTex::drawQuad(double width, double height)
{
	 glBegin(GL_QUADS);
			// top
...
void CMyTex::drawSphere(float rad)
{
  int numMajor = 24;
  int numMinor = 32;
...
void CMyTex::Draw_Ellipse(float x, float y, float z, MyVector scale)
{
//10/27 JEONG
...
//-----------------------------------------------------------------------
void CMyTex::myglSolidFrustum(short Nxy,float ratio)//â~êçë‰
...
void CMyTex::mySolidCylinder(float x, float y, float z, float r, int n)
{
...
float CMyTex::Passed(float oldTime)
{
