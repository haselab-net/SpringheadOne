...
//-----------------------------------------------------------------------
void myglSolidFrustum(short Nxy,float ratio)//‰~‘ä
{   //‰º’ê‚Ì’†S‚ð•¨‘Ì‚Ì’†S
	//’¼ŒaA‚‚³‚P‚Ì‘äŒ`,ã’ê‚Ì•»²½Þ‚Íratio”{,
	double nn[3];//–@ü¬•ª
	float p[32][3]; //’¸“_À•W
	double r; //”¼Œa
	...
	{   theta = theta0*(double)i;
		p[i][0] = (float)(r*cos(theta)*ratio); //ã’ê‚Ìx¬•ª
		p[i][1] = (float)(r*sin(theta)*ratio);       //‚™¬•ª
		p[i][2] = 1.0f;                              //‚š¬•ª
		p[i+Nxy][0] = (float)(r*cos(theta)); //‰º’ê‚Ìx¬•ª
		p[i+Nxy][1] = (float)(r*sin(theta)); //y¬•ª
		p[i+Nxy][2] = 0.0f;                  //‚š¬•ª
	}
