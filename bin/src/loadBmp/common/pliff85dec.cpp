...
/--------------------------------------------------------------------
|
|      $Id: pliff85dec.cpp,v 1.3 2004/04/15 20:22:05 uzadow Exp $
...
void PLIFF85Decoder::Open(PLDataSource * pDataSrc)
{
  ...
  SetBmpInfo(PLPoint(m_bitmapHeader.w, m_bitmapHeader.h), DestBPP, PLPoint(0, 0),
    (m_bitmapHeader.masking == PLIFF85::mskHasTransparentColor), false);
} // Open

void PLIFF85Decoder::GetImage(PLBmp & Bmp)
{
  ...
  // Ensures that the bitmap gets unlocked, even if an exception is thrown.
  class LockHandler
  {
    PLBmp & m_Bmp;
  public:
    explicit LockHandler(PLBmp & rBmp) : m_Bmp(rBmp) {m_Bmp.Lock(false, true);}
    ~LockHandler() {m_Bmp.Unlock();}
  };
  ...
  for (int row = 0; row < m_bitmapHeader.h; ++row)
  {
    ...
    std::vector<PLBYTE> decodedBuf(m_bitmapHeader.w * m_bpp / 8);
    ...
    if (m_formType == PLIFF85::ID_PBM)
    {
      ...
      const int bytes_per_row = m_bitmapHeader.w * m_bpp / 8;
    ...
    std::fill_n(Bmp.GetLineArray()[row], m_bitmapHeader.w * m_bpp / 8, 0);
...
/--------------------------------------------------------------------
|
|      $Log: /Project/Springhead/bin/src/loadBmp/common/pliff85dec.cpp $
 * 
