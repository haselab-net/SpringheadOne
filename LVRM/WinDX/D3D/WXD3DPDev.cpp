...
void WXD3Viewport::TargetSize(SIZE sz){
	begin.clip(WXD3Vec3(0,0,0), WXD3Vec3(1,1,1));
	end.clip(WXD3Vec3(0,0,0), WXD3Vec3(1,1,1));
	WXD3Vec3 size = end - begin;
	size.clip(WXD3Vec3(0,0,0), WXD3Vec3(1,1,1));
	if (end != WXD3Vec3()){
...
WXD3Affine WXD3Viewport::Projection(){
	WXD3Affine afProj;
        ...
        afProj.InitProjectionGL(screen, sz, front, back);
	return afProj;
}
...
bool WXD3DPDevice::EnumDrivers(){
	...
	drivers.clear();
	bool rv = !pWXDD->CKDDERR(pWXDD->pD3D7->EnumDevices(drivers.EnumDevicesCallback, &drivers));
	return rv;
}
...
bool WXD3DPDevice::Create(bool bCallCallback, int nBackBuffer){
	...
	if (!drivers.Select(desc.bitPerPixel, (desc.ddsCaps.dwCaps&DDSCAPS_VIDEOMEMORY)!=0)){
		pWXDD->DDERRMSG(2, "Cannot find d3d driver that is good for this surface.");
		goto releaseReturn;
	}
	...
	if (zBuffer.pDDS && pWXDD->CKDDERR(pDDS->AddAttachedSurface(zBuffer.pDDS))) goto releaseReturn;
	//	create d3ddevice
	if (pD3DDev){
		if (CKDDERR( pD3DDev->SetRenderTarget(pDDS7, 0) )) goto releaseReturn;
	}else{
		CKDDERR( pWXDD->pD3D7->CreateDevice(drivers.CurDriver().guid, pDDS7, &pD3DDev) );
...
bool WXD3DPDevice::Restore(){
	...
	if (rtv && zBuffer.pDDS && zBuffer.pDDS->IsLost() == DDERR_SURFACELOST){
		...
		if (CKDDERRMSG(hr, "restore surface failed.")) return false;
	}
	return rtv;
}
bool WXD3DPDevice::ClearImp(POINT dest, SIZE sz, DWORD color, DWORD flags){
	...
	return !CKDDERR(pD3DDev->Clear(1, (D3DRECT*)&rcDest, D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER, color, 1.0f, 0));
}
void WXD3DPDevice::Viewport(const WXD3Viewport& vp){
	viewport = vp;
	viewport.TargetSize(Size());
	CKDDERR(pD3DDev->SetViewport(&viewport));
	CKDDERR(pD3DDev->SetTransform(D3DTRANSFORMSTATE_PROJECTION, (D3DMATRIX*)&viewport.Projection()));
}
#endif
