...
bool WXD3Device::EnumDrivers(){
	...
	drivers.clear();
	bool rv = true;
	rv &= !pWXDD->CKDDERR(pWXDD->pD3D->EnumDevices(drivers.EnumDevicesCallback, &drivers));
#if DIRECTDRAW_VERSION >= 0x700
	if (pWXDD->pD3D7) rv &= !pWXDD->CKDDERR(pWXDD->pD3D7->EnumDevices(drivers.EnumDevicesCallback, &drivers));
#endif
	return rv;
}
...
bool WXD3Device::Create(bool bCallCallback, int nBackBuffer){
	...
	if (CKDDERR(pWXDD->pD3D->CreateDevice(drivers.CurDriver().guid, pDDS, &pD3DDev))) goto releaseReturn;
#else
	if (CKDDERR(pDDS2->QueryInterface(drivers.CurDriver().guid, (void**)&pD3DDev))) goto releaseReturn;
#endif
...
bool WXD3Device::Restore()
	...
	bool rtv = WXDDFlipSurface::Restore();
  	if (rtv && zBuffer.pDDS && zBuffer.pDDS->IsLost() == DDERR_SURFACELOST)
		...
		if (CKDDERRMSG(hr, "restore surface failed."))
			{
			rtv = false;
			goto leave;
			}
