...
void WBSockAddr::Print(std::ostream& os) const{
	...
	os << ntohs(AdrIn().sin_port) << std::endl;

}
bool WBSockAddr::Set(const char* a, int port){
	HOSTENT* phe=NULL;
	AdrIn().sin_family = AF_INET;
    if(!(phe=gethostbyname(a))){						//	アドレス取得
		int er = WSAGetLastError();
        return false;
    }else{
		memcpy(&AdrIn().sin_addr, phe->h_addr, phe->h_length);   //アドレス値格納
...
void WBNetInterfaces::Init(){
	...
	if (error){
		...
		DSTR << "Error: Failed to query WinSock2, error code" << error << std::endl;
		return;
	}
	for(int i=0; ifInfo[i].iiFlags && i<(nBytesReturned/sizeof(INTERFACE_INFO)); ++i){
