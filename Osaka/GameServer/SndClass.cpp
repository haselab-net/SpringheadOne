...
/*
	if( SndPuckPuck != 0 ){
		...
		LONG vol = (LONG) ( ( ( (float) SndPuckPuck) / MAXSNDPARAM ) * 10000 - 10000 );
		printf( "PuckPuck  %d   %d\n", SndPuckPuck, vol);
		// パック同士の衝突のサウンドを再生する。
		GetSndFile( INDEXSNDPUCKPUCK )->Play( vol );
		// グローバル変数のクリア
		SndPuckPuck = 0;
	}
	if( SndPenPuck != 0 ){
		// volは 最小 -10000 から 最大 0 に範囲変換
		LONG vol = (LONG) ( ( ( (float) SndPenPuck) / MAXSNDPARAM ) * 10000 - 10000 );
		printf( "PenPuck   %d   %d\n", SndPenPuck, vol);
		// パック同士の衝突のサウンドを再生する。
		GetSndFile( INDEXSNDPENPUCK )->Play( vol );
		// グローバル変数のクリア
		SndPenPuck = 0;
	}
	if( SndPuckGoal != 0 ){
		// volは 最小 -10000 から 最大 0 に範囲変換
		LONG vol = (LONG) ( ( ( (float) SndPuckGoal) / MAXSNDPARAM ) * 10000 - 10000 );
		printf( "PuckGoal  %d   %d\n", SndPuckGoal, vol);
		// パック同士の衝突のサウンドを再生する。
		GetSndFile( INDEXSNDPUCKGOAL )->Play( vol );
		// グローバル変数のクリア
		SndPuckGoal = 0;
	}
	if( SndRacPen != 0 ){
		// volは 最小 -10000 から 最大 0 に範囲変換
		LONG vol = (LONG) ( ( ( (float) SndRacPen) / MAXSNDPARAM ) * 10000 - 10000 );
		printf( "RacPen    %d   %d\n", SndRacPen, vol);
		// パック同士の衝突のサウンドを再生する。
		GetSndFile( INDEXSNDRACPEN )->Play( vol );
		// グローバル変数のクリア
		SndRacPen = 0;
	}
	if( SndRacPuck != 0 ){
		// volは 最小 -10000 から 最大 0 に範囲変換
		LONG vol = (LONG) ( ( ( (float) SndRacPuck) / MAXSNDPARAM ) * 10000 - 10000 );
		printf( "RacPuck   %d   %d\n", SndRacPuck, vol);
		// パック同士の衝突のサウンドを再生する。
		GetSndFile( INDEXSNDRACPUCK )->Play( vol );
		// グローバル変数のクリア
		SndRacPuck = 0;
	}
