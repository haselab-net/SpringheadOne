        ...
        if ((hResData = LoadResource(hModule, hResInfo)) != NULL)
			if ((pvRes = LockResource(hResData)) != NULL)
				if ( DSParseWaveResource(pvRes, ppWaveHeader, ppbWaveData, pcbWaveSize))
					...
					OutputDebugString("Sound Resource not check!\n");
			else
				OutputDebugString("Sound Resource not lock!\n");
		else
			OutputDebugString("Sound Resource not load!\n");
	else{
		OutputDebugString("Sound Resource not find! - ");
...
BOOL SndObjPlay(SNDOBJ *pSO, LONG ivolume, DWORD dwPlayFlags)
{
    BOOL result = FALSE;
	...
	if( ivolume > DSBVOLUME_MAX )	ivolume=DSBVOLUME_MAX;
	if( ivolume < DSBVOLUME_MIN )	ivolume=DSBVOLUME_MIN;

    if ((!(dwPlayFlags & DSBPLAY_LOOPING) || (pSO->iAlloc == 1)))
    {
	IDirectSoundBuffer *pDSB = SndObjGetFreeBuffer(pSO);
	if (pDSB != NULL) {
		pDSB->SetVolume( ivolume );
...
/*
//-----------------------------------------------------------------------------
// Name: SndObjSetVolume(LONG ivolume)
// Desc: ボリュームをコントロールする。
//----------------------------------------------------------------------------
BOOL	SndObjSetVolume(SNDOBJ *pS0, LONG ivolume)
{
	BOOL	result = FALSE;
	int i;

	if (pS0 == NULL) return FALSE;

	if( ivolume > DSBVOLUME_MAX )	ivolume=DSBVOLUME_MAX;
	if( ivolume < DSBVOLUME_MIN )	ivolume=DSBVOLUME_MIN;

	for(i=0; i<pS0->iAlloc; i++)
		pS0->Buffers[i]->SetVolume(ivolume);
