...
//
//　N次元連立１次方程式を解く
static void CalcMatRXL(float a[][N], float x[], float b[]){
	int	i,j,k;
	float p,q,s;
	//float x[N];
#ifdef _DEBUG
...
static void CalcAlpha(){
	...
	if(minIndex >= 0){
#ifdef _DEBUG
		printf("%2d(%d -> %d)}\n\r",minIndex,isActiveSet[minIndex],bval);
		printf("alpha = %f (isActiveSet[%d]=%d)\n\r",minAlpha,minIndex,bval);
#endif //_DEBUG
		PRINT_XYD();
...
static void printStatus(){
	...
	printf("X \n\r");
	for(i=0;i<N;i++){
		printf("x[%2d]:%10f  %2d Y[%2d]:%10f \n\r",i,gVecX[i],isActiveSet[i],i,gVecXYNext[N+i]);
	}
	printf("\n\r");
	*/
	//getchar();
