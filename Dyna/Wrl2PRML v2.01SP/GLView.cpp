...
void TGLView::OnOpen(void){
	...
	SetTimer(dt*300, &TGLView::OnTimer);
	if (initialWideScreen) glutReshapeWindow(640,360);
	...
	for(TPenaltyDetectors::iterator it = penChekers.begin(); it != penChekers.end(); ++it){
		...
		for(TPenaltyDetector::TIntersectionSet::iterator itIs = pc.iss.begin(); itIs != pc.iss.end(); ++itIs){
			...
			if (is.convex[0]->convexType()==Convex::POLYTOPE && is.convex[1]->convexType()==Convex::POLYTOPE){
				...
				penMom = Vec3d();
				a = 0.0;
				UsoPenalty(dt, pen, pc, g1, g2, p1, p2, (Vec3d&)is.normal);
...
//				(*it)->UpdateMotion(dt);
		...
		// transmit force to spidar
			...
			v3fAvatarReactionforce = (*it)->force * 0.07;
...
void TGLView::UsoPenalty(double dt, Vec3d pen, TPenaltyDetector& pc, 
							TGeoNode* g1, TGeoNode* g2, Vec3d near1, Vec3d near2, Vec3d& normal){
	...
	Vec3d force = k * (pen + PEN_DAM_STF * (pen - pc.lastPenalty) / dt);
...
#ifndef NON_FRIC
	Vec3d fric;
	pc.applicationPoint = (near1+near2)/2;
...
//	TCalcPenalty cp(newinterior, normal);
	TCalcPenalty cp(interior, normal);
