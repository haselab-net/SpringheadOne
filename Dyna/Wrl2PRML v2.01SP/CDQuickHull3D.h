...
private:
	...
	bool FindFarthest(const Vec3d& normal, double maxDist){
		TVtx** maxVtx=NULL;
		for(TVtx** it=vtxBegin; it!= vtxEnd; ++it){
			double dist = (*it)->Vtx() * normal;
	...
	TVtx** DivideByPlane(const Vec3d& normal, double dist, TVtx** start, TVtx** end){
		while(start != end){
			double d = (*start)->Vtx() * normal;
