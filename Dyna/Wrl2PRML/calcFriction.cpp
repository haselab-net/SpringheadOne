...
Vec3d TCalcFriction::SprPoint(Vec3d& force, Vec3d& torque){
	Matrix3d invForce;
	
	invForce.Ex() = Vec3d(0, force.Z(), -force.Y());
	invForce.Ey() = Vec3d(-force.Z(), 0, force.X());
	invForce.Ez() = Vec3d(force.Y(), -force.X(), 0);
	
	return invForce.trans() * torque;
};

void TCalcFriction::CalcFriction(Vec3d force, Vec3d torque, Vec3d normal, TPenaltyDetector& pc, 
						   		 TGeoNode* g1, TGeoNode* g2, double dt){
	Vec3d gSpPoint[2], currSpP;
	Vec3d fric, dynFric, damper, angFric;
	Affined dPos[2];
	double d = 0.01;
...
//		if ((g1->Vel() - g2->Vel()).norm() < 0.3) fric = Vec3d(0.0, 0.0, 0.0);
		return fric;
	}
