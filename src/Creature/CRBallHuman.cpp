// CRBallHuman.cpp: CRBallHuman クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////
...
SGOBJECTIMP(CRBallHuman, SGBehaviorEngine);
...
bool CRBallHuman::AddChildObject(SGObject* o, SGScene* s){
...
void CRBallHuman::ConnectSolid(const char* name, UTRef<SGScene> scene){
...
void CRBallHuman::ConnectJoint(const char* name, UTRef<SGScene> scene){
	SGScene::SetRange range;
	range = scene->RangeObject(name);
	for(;range.first != range.second; ++range.first){
...
bool CRBallHuman::Connect(UTRef<SGScene> scene){
	solids.clear();
	joints.clear();
	jointPids.clear();
	jointBallPids.clear();
	...
	ConnectJoint("joRHip", scene);
	ConnectJoint("joRKnee", scene);
	ConnectJoint("joRAnkle", scene);
	ConnectJoint("joRToe", scene);

	ConnectJoint("joLHip", scene);
	ConnectJoint("joLKnee", scene);
	ConnectJoint("joLAnkle", scene);
...
bool CRBallHuman::HasFrame(SGFrame* f){
...
void CRBallHuman::SetOneInertia(Spr::PHSolid* solid, Spr::Vec3f axis){
...
void CRBallHuman::SetTotalMass(float mass){
	totalMass = mass;
}
void CRBallHuman::SetTotalHeight(float height){
	totalHeight = height;
}

void CRBallHuman::SetScale(SGScene* scene){
	SetSolidScale();
	SetJointScale();
...
void CRBallHuman::SetSolidScale(){
	SGFrame* transFrame;
	Affinef B;
	B.xx *= totalHeight;
	B.yy *= totalHeight;
...
void CRBallHuman::SetJointScale(){
...
void CRBallHuman::SetJointRange(){
	for(unsigned i = 0; i < joints.size(); i++){
		if(joints[i] != NULL){
...
void CRBallHuman::SetOneJointRange(PHJoint1D* j, float min, float max){
	if(j){
...
void CRBallHuman::JointPIDMul(PHJointPid* jo, float mulP, float mulD){
	jo->proportional *= mulP;
	jo->integral *= mulP;
	jo->differential *= mulD;
}

void CRBallHuman::JointBallPIDMul(PHJointBallPid* jo, float mulP, float mulD){
	jo->proportional *= mulP;
	jo->integral *= mulP;
	jo->differential *= mulD;
}

void CRBallHuman::SetJointSpring(float dt){
...
void CRBallHuman::AddJointPassivityResistance(){
	float torque;
...
Vec3d CRBallHuman::GetCogVelocity(){
	Vec3f p;
	for(unsigned i=0; i<solids.size(); ++i){
		if( solids[i] != NULL){
...
void CRBallHuman::CalcContactForce(){
	supportArea.Step();
	for(int i=0; i<supportObject.size(); ++i){
		supportObject[i].Step();
	}
}
void CRBallHuman::LoadX(const XBallHuman& xh){
	totalHeight = xh.totalHeight;
	totalMass = xh.totalMass;
}
void CRBallHuman::SaveX(XBallHuman& xh) const{
}

class CRBallHumanLoader : public FIObjectLoader<CRBallHuman>
{
public:
	CRBallHumanLoader(){
		...
		db->REG_FIELD(DWORD);
		db->REG_FIELD(FLOAT);
		db->REG_RECORD_PROTO(XBallHuman);
	}
	bool LoadData(FILoadScene* ctx, CRBallHuman* h){
		XBallHuman xh;
...
class CRBallHumanSaver : public FIObjectSaver<CRBallHuman>{
protected:
	void SaveData(class FISaveScene* ctx, FIDocNodeBase* doc, CRBallHuman* h){
		XBallHuman xh;
		h->SaveX(xh);
		doc->SetWholeData(xh);		
	}
