#include "Creature.h"
#pragma hdrstop
// CRBallActionPlanner.cpp: CRBallActionPlanner クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////

#include "CRBallActionPlanner.h"
...
//////////////////// 接触情報クラス ////////////////////
void CRBallActionPlanner::ContactInfo::SetContactInfo(int vh, int user, int c, float t){
	soVHIndex = vh;
	soUserIndex = user;
...
int CRBallActionPlanner::ContactInfo::CheckContactType(int vh, int user){
	// 普通の接触
	int type = 0;
...
//////////////////// 予測行動クラス ////////////////////
CRBallActionPlanner::CRBallActionPlanner(){
}
	
void CRBallActionPlanner::Load(){
	bDraw       = false;
	bPlanner    = false;
	bPrediction = false;
	bLoadReal   = false;
...
void CRBallActionPlanner::SaveState(SGScene* scene, bool type){
	if(type){
		stateReal.clear();
...
void CRBallActionPlanner::LoadState(SGScene* scene, bool type){
	if(type){
		if(bLoadReal){
...
void CRBallActionPlanner::Step(CRBallPuppet* puppet, CRBallUser* user, SGScene* scene){
...
void CRBallActionPlanner::PredictionAction(CRBallPuppet* puppet, CRBallUser* user, SGScene* scene){
...
void CRBallActionPlanner::MovementPrediction(CRBallPuppet* puppet, CRBallUser* user, SGScene* scene, int count){
...
void CRBallActionPlanner::PredictionStep(CRBallPuppet* puppet, CRBallUser* user, SGScene* scene, int count){
	float dt = scene->GetTimeStep();
...
bool CRBallActionPlanner::IsFirstContact(int vh, int user, int count){
...
void CRBallActionPlanner::ContactTest(CRBallPuppet* puppet1, CRBallPuppet* puppet2, SGScene* scene, int step){
	ContactInfo cInfo;
...
bool CRBallActionPlanner::ChooseTargetAction(CRBallPuppet* puppet, CRBallUser* user){
	int cType;
...
Vec3f CRBallActionPlanner::GetNearestPoint(Vec3f a, Vec3f b, Vec3f c){
	// 線分ab上で点cとの最近傍点を返す
	Vec3f p;
	float t;
...
Vec3f CRBallActionPlanner::GetPointToAvoid(Vec3f a, Vec3f b, Vec3f c, float d){
...
Vec3f CRBallActionPlanner::GetPointToGuard(Vec3f a, Vec3f b, Vec3f c){
