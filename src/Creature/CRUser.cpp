...
void CRUser::SetSprings(){
	...
	// [0] 腰(基本立ち位置に固定)
	postureSpring.SetSolid(solids[0], 1.0f, 1.0f);
	//positionSpr.SetSolid(solids[0], Vec3f(0, 0, 0), 0.01f, 0.01f);
	positionSpr.SetSolid(solids[0], Vec3f(0, 0, 0), 0.01f, 0.1f);
	positionSprings.push_back(positionSpr);
	// [1] 右手(Spidarに対応)
	positionSpr.SetSolid(solids[6],  Vec3f(0, 0, 0), 0.1f, 0.1f);
	positionSprings.push_back(positionSpr);
	// [2] 左手
	positionSpr.SetSolid(solids[9],  Vec3f(0, 0, 0), 0.1f, 0.1f);
	positionSprings.push_back(positionSpr);
	// [3] 頭頂(ラベリングカメラに対応)
...
void CRUser::SetExpectedPos(float dt){
