========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : DDrawSDIEx
========================================================================


AppWizard が生成した DDrawSDIEx には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには DDrawSDIEx を構成している各ファイルの概要説明が含まれています。

DDrawSDIEx.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また WXDDrawSDIExApp アプリケーションクラスを宣
    言します。

DDrawSDIEx.cpp
    このファイルは WXDDrawSDIExApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\DDrawSDIEx.ico
    ...
    ルです。このアイコンはリソース ファイル DDrawSDIEx.rc によってインクルード
    されます。
...
DDrawSDIExDoc.h, DDrawSDIExDoc.cpp - ドキュメント
    これらのファイルは WXDDrawSDIExDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、WXDDrawSDIExDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

DDrawSDIExView.h, DDrawSDIExView.cpp - ビュー
    これらのファイルは WXDDrawSDIExView クラスを含みます。
    WXDDrawSDIExView オブジェクトは WXDDrawSDIExDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) DDrawSDIEx.pch 
