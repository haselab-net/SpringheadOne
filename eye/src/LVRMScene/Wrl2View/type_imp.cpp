...
void TTypeTransform::Init(TNode *pn)
{
	float		center[] = {0, 0, 0};
	float		translation[] = {0, 0, 0};
	float		rotation[] = {0, 0, 1, 0};
...
void TTypePointLight::Draw(TNode *pn)
{
	...
	CopyElm(pn, "ambientIntensity", &ambientIntensity);
	CopyElm(pn, "attenuation", attenuation);
	CopyElm(pn, "color", color);
	CopyElm(pn, "location", location);
	CopyElm(pn, "on", &on);
	CopyElm(pn, "radius", &radius);
	static int count=GL_LIGHT0;
	Vec3f ambient = ambientIntensity * (Vec3f&)color;
	glLightfv(count, GL_AMBIENT, ambient);
	glLightfv(count, GL_DIFFUSE, color);
	glLightfv(count, GL_SPECULAR, color);
	glLightf(count, GL_CONSTANT_ATTENUATION, attenuation[0]);
	glLightf(count, GL_LINEAR_ATTENUATION, attenuation[1]);
	glLightf(count, GL_QUADRATIC_ATTENUATION, attenuation[2]);
	glLightfv(count, GL_POSITION, location);
	if (on) glEnable(count);
	else glDisable(count);
}
