/*
|
|      $Id: plpicenc.cpp,v 1.7 2003/08/03 12:03:20 uzadow Exp $
...
void PLPicEncoder::MakeFileFromBmp (const char * pszFName, PLBmp * pBmp)
{
  ...
  int bufsize = pBmp->GetMemUsed() + 65536;
...
#ifdef _WINDOWS
void PLPicEncoder::MakeFileFromBmpW (const wchar_t * pszwFName, PLBmp * pBmp)
{
  PLFileSink FileSink;
  int err;
...
/--------------------------------------------------------------------
|
|      $Log: /Project/Springhead/bin/src/loadBmp/common/plpicenc.cpp $
 * 
