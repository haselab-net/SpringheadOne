...
GLOBAL Trealf        rnd_wave = 0., yflow = 0.0, flow_scl = 1.0;
...
GLOBAL Trealf frc_wav_scl = 1.0, frc_arrw_scl = 1.0;
...
void calPressure(ThapticObj *po, int use_vel);
void genRandWave(Trealf wx, Trealf wy, Trealf amp);
...
//            istereo = 0;
            ...
            i++;
        } else if(!strcmp(argv[i], "-frc_disp_scl")) {
            sscanf(argv[i+1], "%f", &frc_wav_scl);
            i++;
            sscanf(argv[i+1], "%f", &frc_arrw_scl);
            i++;
            ...
            i++;
        } else if(!strcmp(argv[i], "-spidar_file")) {
            sscanf(argv[i+1], "%s", spidar_file);
            i++;
...
//    initSound();
    ...
    if(use_spidar == TRUE) {
        ...
        force_abs = (Trealf *)malloc(sizeof(Trealf)*nforce_max);
        torq_abs = (Trealf *)malloc(sizeof(Trealf)*nforce_max);
...
*/
    ...
    vlight.y = light_pos[1] = 0.9/sqrt(1.);
    vlight.z = light_pos[2] = -0.1/sqrt(1.);
    light_pos[3] = 0.001;
                    ...
                    if(iwx>0 && iwx<MX-1) {
                        ...
                        if(r < 1.0){
							...
							if (tmp[IX(iwx,iwy)] + w < limit){
								...
								pw[IX(iwx,iwy)] += p*w;
								if (w>0 && use_vel == TRUE && po->hsrc[k].a > 0){
									float sw = 1-w;
									sw = sw*sw*sw;
									if (sw<0) sw = 0;
...
void calPressure(ThapticObj *po, int use_vel) {
    ...
    for(k=0; k<po->nhsrc; k++) {
        cx = po->hsrc[k].x;//+po->hsrc[k].vx*20;
        cy = po->hsrc[k].y+dh;//+po->hsrc[k].vy*20;
        p = 0.0;
        p = po->hsrc[k].prs.x*po->hsrc[k].prs.x
           +po->hsrc[k].prs.y*po->hsrc[k].prs.y
           +po->hsrc[k].prs.z*po->hsrc[k].prs.z;
        if(p > 0.0) {
//            p = po->pres_scl*sqrt(p)*32000;
            p = sqrt(p);
//        if(po->hsrc[k].prs.y < 0.0) p = -p;
            xh = cx/dh + MX/2.;
            yh = cy/dh + MY/2.;
            for(j= -RRR; j<=RRR; j++) {
                iwy = yh + j;
...
void genRandWave(Trealf wx, Trealf wy, Trealf amp) {
    ...
    cx = wx;//+po->hsrc[k].vx*20;
    cy = wy+dh;//+po->hsrc[k].vy*20;
    xh = cx/dh + MX/2.;
    yh = cy/dh + MY/2.;
    for(j= -SSS; j<=SSS; j++) {
        iwy = yh + j;
...
//            height[IX(i,j)]=wh1[IX(i,j)]*loss * hmul;
    ...
    for(i=1;i<=MX-2;i++){
        for(j=1;j<=MY-2;j++){
            u1[IX(i,j)]=u[IX(i,j)]-gravity*(dt/dh)*(height[IX(i+1,j)]-height[IX(i,j)])+(p[IX(i+1,j)]-p[IX(i,j)])/(rho_w*dh);
            v1[IX(i,j)]=v[IX(i,j)]-gravity*(dt/dh)*(height[IX(i,j+1)]-height[IX(i,j)])+(p[IX(i,j+1)]-p[IX(i,j)])/(rho_w*dh);
        }
    }
    for(i=1;i<=MX-2;i++){
        for(j=1;j<=MY-2;j++){
